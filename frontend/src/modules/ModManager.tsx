...
interface ModsStatus {
  ...
  dashboard_mod_installed: boolean
  mods_count: number
...
const ModManager: React.FC = () => {
  ...
  const [installingDashboardMod, setInstallingDashboardMod] = useState(false)
  ...
  const handleInstallDashboardMod = async () => {
    setInstallingDashboardMod(true)
    setError('')
    setSuccess('')

    try {
      const response = await api.post('/api/mods/install-dashboard-mod')
      setSuccess(response.data.message)
      fetchStatus()
    } catch (error: any) {
      setError(error.response?.data?.error || 'Dashboard mod installation failed')
    } finally {
      setInstallingDashboardMod(false)
    }
  }
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
            ...
            <div className="bg-white rounded-lg shadow-sm p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Dashboard Mod</p>
                  <p className={`text-2xl font-bold ${status.dashboard_mod_installed ? 'text-success-600' : 'text-error-600'}`}>
                    {status.dashboard_mod_installed ? 'Installed' : 'Missing'}
                  </p>
                </div>
                <div className={`p-3 rounded-full ${status.dashboard_mod_installed ? 'bg-success-50' : 'bg-error-50'}`}>
                  <Settings className={`h-6 w-6 ${status.dashboard_mod_installed ? 'text-success-600' : 'text-error-600'}`} />
                </div>
              </div>
            </div>
          </div>
          ...
          {/* Dashboard Mod Section */}
          <div className="bg-white rounded-lg shadow-sm p-6">
            <h3 className="text-lg font-semibold text-gray-900 mb-4">Dashboard Mod</h3>
            {status.dashboard_mod_installed ? (
              <div className="flex items-center space-x-2 text-success-600">
                <CheckCircle className="h-5 w-5" />
                <span className="font-medium">Dashboard mod installed</span>
              </div>
            ) : (
              <div>
                <p className="text-gray-600 mb-4">
                  Install the custom dashboard mod to enable advanced server integration features.
                </p>
                <button
                  onClick={handleInstallDashboardMod}
                  disabled={installingDashboardMod}
                  className="flex items-center space-x-2 bg-accent-600 text-white px-4 py-2 rounded-md hover:bg-accent-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                >
                  {installingDashboardMod ? (
                    <Loader2 className="h-4 w-4 animate-spin" />
                  ) : (
                    <Download className="h-4 w-4" />
                  )}
                  <span>Install Dashboard Mod</span>
                </button>
              </div>
            )}
          </div>
