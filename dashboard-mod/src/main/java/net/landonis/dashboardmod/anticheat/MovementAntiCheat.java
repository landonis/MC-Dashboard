...
public class MovementAntiCheat {
    ...
    private static class PlayerMovementData {
        final Queue<MovementSnapshot> positionHistory = new LinkedList<>();
        Vec3d lastValidPosition;
    ...
    private boolean checkFlyViolation(ServerPlayerEntity player, PlayerMovementData data, double verticalDistance) {
        ...
        double maxStep = getMaxStepHeight(player);

        // Allow walking up stairs / slabs
        if (verticalDistance > 0 && verticalDistance <= maxStep && player.isOnGround()) return false;
        ...
        // Legitimate jumps
        if (!player.isOnGround() && verticalDistance <= maxVertical * LAG_COMPENSATION_MULTIPLIER) return false;
    ...
    private double getMaxStepHeight(ServerPlayerEntity player) {
    ...
    private boolean checkPhaseViolation(ServerPlayerEntity player, Vec3d fromPos, Vec3d toPos) {
        ...
        if (distance > 0.1) {
            ...
            for (int i = 1; i < steps; i++) {
                ...
                if (!world.getBlockState(blockPos).isAir() && world.getBlockState(blockPos).getBlock() != Blocks.WATER) {
                    if (wouldCollide(player, checkPos)) {
                        recordViolation(getPlayerData(player.getUuid()), player, "Phase/NoClip through " + world.getBlockState(blockPos).getBlock());
                        return true;
                    }
                }
            }
    ...
    private boolean checkJesusViolation(ServerPlayerEntity player, Vec3d position) {
        ...
        if (world.getBlockState(below).getBlock() == Blocks.WATER && player.isOnGround() && !player.getAbilities().allowFlying) {
    ...
    private double getMaxAllowedSpeed(ServerPlayerEntity player) {
        ...
        if (player.isCreative() || player.isSpectator()) baseSpeed = player.getAbilities().getFlySpeed() * 20;
    ...
    private void recordViolation(PlayerMovementData data, ServerPlayerEntity player, String reason) {
        ...
        System.out.println("[AntiCheat] Movement violation by " + player.getName().getString() + ": " + reason + " (Total: " + data.violationCount + ")");
    ...
    private void remediate(ServerPlayerEntity player, PlayerMovementData data, String reason) {
        ...
        if (data.violationCount == 5) player.sendMessage(net.minecraft.text.Text.of("§6[AntiCheat] §eMovement irregularities detected"));
        if (data.violationCount == 10) player.sendMessage(net.minecraft.text.Text.of("§c[AntiCheat] §cSuspicious movement patterns detected"));
